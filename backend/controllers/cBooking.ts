...
const cBooking = {
    ...
    createBooking: async (request: Request, response: Response) => {
        try {
            ...
            const result = await withTransaction(async (conn: PoolConnection) => {
                ...
                const [spaces] = await conn.query<RowDataPacket[]>(
                    `SELECT s.spaces_id, st.priceHour, st.priceDay, st.membershipType_id, st.capacity
                     FROM Spaces s
                     INNER JOIN SpacesTypes st ON s.spacesType_id = st.spacesType_id
                     WHERE s.spaces_id = ? FOR UPDATE`,
                    [spaces_id]
                );
                if (!spaces[0]) throw new AppError(404, "Espacio no encontrado");

                // 2. Verificar membresía del usuario
                const [users] = await conn.query<RowDataPacket[]>(
                    "SELECT membershipType_id FROM Users WHERE user_id = ?",
                    [user_id]
                );
                if (!users[0]) throw new AppError(404, "Usuario no encontrado");
                ...
                const [overlaps] = await conn.query<RowDataPacket[]>(
                    `SELECT 1 FROM Bookings
                     ...
                     LIMIT 1`,
                    [spaces_id, timeDate, until, timeFrom]
                );
                ...
                const [bookingResult] = await conn.query<ResultSetHeader>(
                    `INSERT INTO Bookings 
                     ...
                     VALUES (?, ?, 2, ?, NOW(), ?, ?, ?)`,
                    [user_id, spaces_id, numberPersons || 1, timeDate, timeFrom, until]
                );

                // 6. INSERT Pay (atómico con el booking)
                const [payResult] = await conn.query<ResultSetHeader>(
                    "INSERT INTO Pays (booking_id, statusPay_id, totalPay) VALUES (?, 1, ?)",
                    [bookingResult.insertId, pricing.total]
                );
     ...
     */
    getBookings: async (request: Request, response: Response) => {
        try {
            const user_id = (request as any).user.id;
     ...
     */
    getBooking: async (request: Request, response: Response) => {
        try {
            const user_id = (request as any).user.id;
        ...
        try {
            ...
            const result = await withTransaction(async (conn: PoolConnection) => {
                ...
                const [bookings] = await conn.query<RowDataPacket[]>(
                    `SELECT b.*, p.totalPay, p.statusPay_id
                     FROM Bookings b
                ...
                if (!bookings[0]) throw new AppError(404, "Reservación no encontrada. Asegúrese de haber ingresado los datos de una reserva que usted haya realizado.");
                if (bookings[0].statusbooking_id === 3) throw new AppError(409, "Esta reserva ya fue cancelada anteriormente.");
                if (bookings[0].statusbooking_id === 4) throw new AppError(409, "No se puede cancelar una reserva que ya fue completada.");
                ...
                await conn.query(
                    "UPDATE Bookings SET statusbooking_id = 3 WHERE booking_id = ?",
                    [booking_id]
                );

                // 4. INSERT registro de reembolso
                await conn.query(
                    "INSERT INTO Refunds (dateAct, refundPercent, booking_id, totalRefund) VALUES (NOW(), ?, ?, ?)",
                    [refundPercent, booking_id, totalRefund]
                );

                // 5. UPDATE estado del pago a refund
                if (refundPercent > 0) {
                    await conn.query(
                        "UPDATE Pays SET statusPay_id = 2 WHERE booking_id = ?",
                        [booking_id]
                    );
                }
     ...
     */
    extendBooking: async (request: Request, response: Response) => {
        try {
            const user_id = (request as any).user.id;
            const booking_id = request.params.id;
            ...
            const result = await withTransaction(async (conn: PoolConnection) => {
                ...
                const [bookings] = await conn.query<RowDataPacket[]>(
                    `SELECT b.*, st.priceHour, st.membershipType_id as spaceMembershipTypeId,
                ...
                if (!bookings[0]) throw new AppError(404, "Reservación no encontrada");
                if (bookings[0].statusbooking_id !== 2) throw new AppError(409, "Solo se pueden extender reservaciones confirmadas.");
                ...
                const [overlaps] = await conn.query<RowDataPacket[]>(
                    `SELECT 1 FROM Bookings
                     WHERE spaces_id = ? AND timeDate = ?
                       ...
                       AND timeFrom < ? AND until > ?
                     LIMIT 1`,
                    [bookings[0].spaces_id, timeDateStr, booking_id, until, bookings[0].until]
                );
                if (overlaps.length > 0) {
                ...
                // 4. UPDATE booking
                await conn.query(
                    "UPDATE Bookings SET until = ? WHERE booking_id = ?",
                    [until, booking_id]
                );

                // 5. UPDATE pago
                await conn.query(
                    "UPDATE Pays SET totalPay = ? WHERE booking_id = ?",
                    [pricing.total, booking_id]
                );
