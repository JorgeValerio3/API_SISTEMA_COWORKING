...
const cAdmin = {
    getBookings: async (request: AuthenticatedRequest, response: Response) => {
        try {
    ...
    getAllStats: async (request: AuthenticatedRequest, response: Response) => {
        try {
            const user_id = request.user!.id;
    ...
    extendBooking: async (request: AuthenticatedRequest, response: Response) => {
        try {
            const user_id = request.user!.id;
            ...
            if (!booking) return error.e404(request, response, { message: "Booking not found" });
            ...
            if (!isAvailable) return error.e409(request, response, {
