...
}).refine(data => new Date(`${data.timeDate}T${data.until}`) > new Date(`${data.timeDate}T${data.timeFrom}`), { message: "'until' debe ser posterior a 'timeFrom'", path: ["until"] }).refine(data => data.timeFrom >= "08:00:00", { message: "No se puede reservar antes de las 08:00 AM", path: ["timeFrom"] }).refine(data => data.until <= "21:00:00", { message: "No se puede reservar después de las 09:00 PM", path: ["until"] });
