...
const vBookings = {
    validateId: async (request: Request, response: Response, next: NextFunction) => {
        try {
            const id = request.params.id;
            if(!id) return error.e400(request, response, {
                message: "Valores Incompletos por favor enviar todos los valores para realizar la busqueda."
            });
            if(validateData.validateNumber(id)) return error.e400(request, response, {
    ...
    validateCreateBooking: async (request: Request, response: Response, next: NextFunction) => {
        try {
            const { spaces_id, timeDate, timeFrom, until } = request.body;
            if(!timeDate || !timeFrom || !until || !spaces_id) return error.e400(request, response, {
                message: "Valores Incompletos por favor enviar todos los valores para Realizar la Consulta."
            });
            if(validateData.validateDate(timeDate)) return error.e400(request, response, {
            ...
            if(validateData.validateTime(timeFrom)) return error.e400(request, response, {
            ...
            if(validateData.validateTime(until)) return error.e400(request, response, {
            ...
            if(validateData.validateNumber(spaces_id)) return error.e400(request, response, {
            ...
            if (new Date(`${timeDate}T${timeFrom}`) <= new Date(`${timeDate}T08:00:00`)) return error.e400(request, response, {
                message: "El campo 'timeDate' esta fuera de rango, no puedes hacer reservacion antes de las 8:00AM"
            });      
            if (new Date(`${timeDate}T${until}`) >= new Date(`${timeDate}T21:00:00`)) return error.e400(request, response, {
                message: "El campo 'timeDate' esta fuera de rango, no puedes hacer reservacion despues de las 9:00PM"
            });      
            if (new Date(`${timeDate}T${until}`) <= new Date(`${timeDate}T${timeFrom}`)) return error.e400(request, response, {
    ...
    validateExtendBooking: async (request: Request, response: Response, next: NextFunction) => {
        try {
            const { until } = request.body;
            if(!until) return error.e400(request, response, {
                message: "Valores Incompletos por favor enviar todos los valores para Realizar la Consulta."
            });
            if(validateData.validateTime(until)) return error.e400(request, response, {
                message: `Valores Incompatibles, until: ${until} no es un valor valido por favor ingrese un tipo de dato valido en formato TIME en el until.`
